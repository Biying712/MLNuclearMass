...
class MPNNLayer(MessagePassing):
    def __init__(self, input_size,num_hidden_layers=2, hidden_feature=256, output_size=256):
        super(MPNNLayer, self).__init__(aggr='mean')  # "Add" aggregation.
        layers=[]

        layers.append(nn.Linear(2*input_size, hidden_feature))
        ...
        #     nn.Linear(2 * input_size, hidden_feature),
        #     nn.PReLU(),
        #     nn.Linear(hidden_feature, output_size)
        # )
    ...
    def message(self, x_i, x_j, edge_index, size):
        # Construct messages
        # x_i is the central node being updated, and x_j are the neighboring nodes
        tmp = torch.cat([x_i, x_j], dim=1)  # Concatenate features
        return self.mlp(tmp)
...
class MPNNModel(nn.Module):
    def __init__(self, input_size, num_hidden_layers=8,hidden_feature=256, output_size=1):
        ...
        self.mpnn1 = MPNNLayer(input_size, hidden_feature=hidden_feature,output_size=hidden_feature)
        self.mpnn2 = MPNNLayer(hidden_feature, hidden_feature=hidden_feature, output_size=hidden_feature)
        self.mpnn3 = MPNNLayer(hidden_feature, hidden_feature=hidden_feature, output_size=hidden_feature)
        self.mpnn4 = MPNNLayer(hidden_feature, hidden_feature=hidden_feature, output_size=output_size)
        self.prelu=nn.PReLU()
    def forward(self, data):
        ...
        x = self.prelu(x)
        x = self.mpnn3(x, edge_index)
        x = self.prelu(x)
        x = self.mpnn4(x, edge_index)

        return x


class MassPrediction_MPNN(MassPrediction):
    ...
    def model_prepare(self):
        self.model = MPNNModel(input_size=self.x_train.shape[1]).to(self.device)
